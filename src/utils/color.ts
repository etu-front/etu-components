...
function getNumberByColor(color: string) {
  // hex 类型颜色
  if (color[0] === '#') {
  ...
  return parseInt(`${Number(match[1]).toString(16)}${Number(match[2]).toString(16)}${Number(match[3]).toString(16)}`,
    16)
}
...
function lightenDarkenColor(color: string, factor?: number) {
  ...
  if (typeof num === 'boolean') return color
  ...
  if (usePound) return `#${(g | (b << 8) | (r << 16)).toString(16)}`
  return `rgb(${r},${g},${b})`
}

export const lighten = (color: string, factor: number) => lightenDarkenColor(color, Math.abs(factor))

export const darken = (color: string, factor: number) => lightenDarkenColor(color, -Math.abs(factor))
